...
def fetch_calendar(**context):
    ...
    params = {
        ...
        "to":  (logical_date + timedelta(days=7)).strftime('%Y-%m-%d'),
        "token": api_key
    }
    ...
    context['ti'].xcom_push(key='calendar_data', value=data)
        
def insert_calendar(**context):
    
    # pull from XCom
    payload = context['ti'].xcom_pull(
    task_ids='fetch_finnhub_calendar',
    key='calendar_data'
    ) or {}
